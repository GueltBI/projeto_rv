...
def get_filtered_data(conta=None, produto=None, ativo=None):
    ...
    # Ajustes nas colunas
    df = df.sort_values(by=['ativo', 'conta'])
    ...
    df = df[['conta','ativo','qtde','preco_medio',
             'produto','preco_atual','ganho_financeiro',
             'rentabilidade']]
    ...
    ##### Consulta separada para obter códigos de cliente distintos
    connection = psycopg2.connect(
        user="gueltdbmaster",
        password="gueltdbpassword123",
        host="gueltdatabase-01.c3qxaey67bfc.us-east-1.rds.amazonaws.com",
...
def get_filtered_data2(id_cliente=None, produto=None, ativo=None):
    ...
    if not plan:
        ...
        return pd.DataFrame(columns=[
            'id_cliente', 'Data de Entrada', 'ativo', 'produto', 'ganho_financeiro', 'divi_total', 'Data de Saída'
        ])
    ...
    connection = psycopg2.connect(user="gueltdbmaster",
                                password="gueltdbpassword123",
                                host="gueltdatabase-01.c3qxaey67bfc.us-east-1.rds.amazonaws.com",
                                port="5432",
                                database="postgres",
...
##############################################################################################################
    ...
    # Selecionando as linhas com os índices obtidos
    dff = dff.loc[idx]
...
def protected_view(request):
    ...
    # Calcular rentabilidade, evitando divisão por zero; Parte do código para uso de setas de rentabilidade::
    def calc_rentabilidade(preco_medio, ganho_financeiro, qtde):
        if preco_medio == 0 or pd.isnull(preco_medio):
            return None
    ...
    #data['preco_medio'] = data['preco_medio'].apply(lambda x: f"R$ {x:,.2f}")
    data['preco_atual'] = data['preco_atual'].apply(lambda x: f"R$ {x:,.2f}")
    #data['rentabilidade'] = data['rentabilidade'].apply(lambda x: f"{x:,.2f}%")
    ...
    #table_html = data.to_html(classes="table table-dark table-striped", index=False, justify='center')
